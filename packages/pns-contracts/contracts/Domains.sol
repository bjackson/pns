...
contract Domains is ERC721URIStorage {
  ...
  modifier onlyDomainOwner(string calldata domainName) {
    if (msg.sender != domains[domainName]) revert Unauthorized();
    _;
  }

  function priceToRegisterDomain(string calldata domainName) pure public returns (uint) {
    uint len = StringUtils.strlen(domainName);
  ...
  function register(string calldata domainName) public payable {
    if (domains[domainName] != address(0)) revert AlreadyRegistered();
    if (!isValidDomainName(domainName)) revert InvalidName(domainName);

    uint _price = priceToRegisterDomain(domainName);
  ...
  function getAddress(string calldata domainName) public view returns (address) {
    return domains[domainName];
  }

  function setRecord(string calldata domainName, string calldata record) onlyDomainOwner(domainName) public {
    records[domainName] = record;
    console.log("%s has set the record for %s to %s", msg.sender, domainName, record);
  }
